...
@javax.inject.Named(
    value ="TestServiceServer")
class TestServiceServer @javax.inject.Inject constructor(
  private val serviceImpl: TestService,
  ...
  override val service: String = TestService.SERVICE
    ...
    try {
      ...
      return when (method) {
        TestService.METHOD_REQUEST_RESPONSE -> {
          ...
          serviceImpl.requestResponse(io.wisoft.test.Test.Request.parseFrom(inputStream), metadata).map(serializer).toFlowable().compose(requestResponseTrace(spanContext)).firstOrError()
        }
